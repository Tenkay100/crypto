...
function loadFirebaseSDK() {
    return new Promise((resolve) => {
        ...
        appScript.onerror = () => {
            console.warn("Failed to load Firebase App compat script");
            resolve();
        };
        appScript.onload = () => {
            ...
            fsScript.onerror = () => {
                console.warn("Failed to load Firebase Firestore compat script");
                resolve();
            };
            fsScript.onload = () => {
                try {
...
const Auth = {
    ...
    async init() {
        ...
        this.initPromise = (async () => {
            ...
            if (firebaseDb) {
                try {
                    // Set a 3.5-second timeout racer to prevent connection hangs from freezing login page
                    const timeoutPromise = new Promise((_, reject) =>
                        setTimeout(() => reject(new Error('Connection timed out')), 3500)
                    );
                    const doc = await Promise.race([
                        firebaseDb.collection('system').doc('usersData').get(),
                        timeoutPromise
                    ]);
                    ...
                    if (doc.exists) {
                        ...
                        if (data && data.users && Object.keys(data.users).length > 0) {
                            ...
                            // Merge local database with cloud database
                            const merged = { ...localUsers, ...data.users };
                            localStorage.setItem('users', JSON.stringify(merged));
                            
                            // If local has accounts that aren't on the cloud, push them up
                            let hasNewLocalAccounts = false;
                            Object.keys(localUsers).forEach(k => {
                                if (!data.users[k]) {
                                    hasNewLocalAccounts = true;
                                }
                            });
                            
                            if (hasNewLocalAccounts) {
                                syncToFirebase();
                            }
                        }
                    } else {
            ...
            if (!users || typeof users !== 'object' || Object.keys(users).length === 0) {
                ...
                // Force update or create default accounts to prevent login issues
                let updated = false;
                ...
                if (!users[DEMO_USER.email]) {
                    ...
                    updated = true;
                } else if (users[DEMO_USER.email].passwordHash !== DEMO_USER.passwordHash) {
